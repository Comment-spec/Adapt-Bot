...
class Bot(discord.Client):
	def __init__(self, dict, controller):
		...
		self.command_list = ['!absent', '!change', '!help', '!commands', '!info', '!raid', '!guildlraid' '!guild', '!invites', '!advertise', '!raidcheck', '!roleassign', '!pugpurge']
	...
	async def on_message(self, message):
		...
		async def graid(message):
				await self.graid_conversation(message.author)
		...
		async def help_command_channel():
			await self.message(self.command_channel, '**`!invites`** - Sends a message to everyone that has been added to the roster for one of the raids, letting them know they were invited.' +
				...
				'\n**`!guildraid`** - Apply to a guild raid.')
		...
		elif message.content == '!guildraid':
			await graid(message)
	...
	async def raid_conversation(self, user):
		...
		try:
			while(conversing):
				...
				conversing, type = await self.conversation('Which type would you like to apply for? Please select a raid. **MC**, **BWL**, or **Cancel** (case is not important)', user, check, 120.0)
				type.content = type.content.lower()
				while type.content != 'mc' and type.content != 'bwl' and type.content != 'cancel':
					conversing, type = await self.conversation('Please provide one of the following valid commands:\n**MC** - Apply to one of our MC raids.\n**BWL** - Apply to one of our BWL raids.\n**Cancel** - Cancel the application.', user, check, 120.0)
					type.content = type.content.lower()
				if type.content == 'cancel':
					type.content = 'Cancel'
				if type.content == 'mc':
					type.content = 'MC'
				if type.content == 'bwl':
				...
				if type.content == 'BWL':
					...
					conversing, character_guild = await self.conversation('What is the name of the Guild that your character is in?', user, check, 60.0)
					application.update({'guild': character_guild.content})
					conversing, soft_res_one = await self.conversation('What is the item you would like to soft reserve?', user, check, 120.0)
					application.update({'soft_res_one': soft_res_one.content})
					#conversing, soft_res_two = await self.conversation('What is the second item that you would like to soft reserve?', user, check, 120.0)
					#application.update({'soft_res_two': soft_res_two.content})
					application.update({'discord_name': '{0}#{1}'.format(user.name, user.discriminator)})
	...
	async def graid_conversation(self, user):
		def check(response):
			return response.channel == user.dm_channel and response.author == user
		def yes_check(response):
			return response.channel == user.dm_channel and (response.content == 'yes' or response.content == 'Yes') and response.author == user
		...
		try:
			while(conversing):
				await self.message(user, StringMachine.graids_recruitment(my_raiding.raid_list, my_raiding.metadata))

				conversing, type = await self.conversation('If you would like to apply to one of the guild runs please respond with **Orange** or **Purple**. Otherwise type **cancel** to stop the application.', user, check, 120.0)
				type.content = type.content.lower()
				while type.content != 'orange' and type.content != 'purple' and type.content != 'cancel':
					conversing, type = await self.conversation('Please provide one of the following valid commands:\n**Orange** - Apply to the raids under Orange team.\n**Purple** - Apply to the raids under purple team.\n**Cancel** - Cancel the application.', user, check, 120.0)
				if type.content == 'cancel':
					type.content = 'Cancel'
				if type.content == 'orange':
					type.content = 'Orange'
				if type.content == 'purple':
					type.content = 'Purple'
				if type.content == 'Cancel':
					await self.message(user, 'Your application has been cancelled!')
					return
				...
				conversing, character_role = await self.conversation('What is your character\'s role? (DPS, Tank, Healer)', user, check, 60.0)
				application.update({'role': character_role.content})
				conversing, soft_res_one = await self.conversation('What item would like to soft reserve for BWL?', user, check, 120.0)
				#conversing, character_guild = await self.conversation('What team are you usually a part of: Orange or Purple?', user, check, 60.0)
				#application.update({'guild': character_guild.content})
				application.update({'soft_res_one': soft_res_one.content})
				conversing, soft_res_two = await self.conversation('What is the item that you would like to soft reserve for MOLTEN CORE (If attending right after BWL, otherwise N/A)?', user, check, 120.0)
				application.update({'soft_res_two': soft_res_two.content})
				application.update({'discord_name': '{0}#{1}'.format(user.name, user.discriminator)})
...
class StringMachine:
	...
	def raids_info(raid_list, metadata):
		...
		for type_key in metadata.get('types').keys():
			if type_key == 'MC' or type_key == 'BWL':
	...
	def graids_recruitment(raid_list, metadata):
		str = 'Hello!\n<Adapt> is hosting the following Raids:\n{0}'.format(StringMachine.graids_info(raid_list, metadata))
		return str

	def graids_info(raid_list, metadata):
		str = ''
		for type_key in metadata.get('types').keys():
			if type_key == 'Orange' or type_key == 'Purple':
				str += StringMachine.raids_info_for_type(raid_list, metadata, type_key) + '\n\n'
		# Remove last \n
		if str != '':
			str = str[:-2]
